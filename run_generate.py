...
from gui.utils.db_local import append_security_audit
...
class GeneratorManagerApp(ctk.CTk):
    ...
    def _build_right_panel(self):
        ...
        ctk.CTkButton(
            btn_tools,
            text="📂 Mở thư mục Barcode",
            fg_color=_S.SUCCESS,
            hover_color=_S.SUCCESS_DARK,
            font=_S.BODY_B,
            height=42,
            command=self._open_barcode_folder,
        ).pack(side="left", padx=(0, 10))
        ...
        def _task():
            try:
                with _connect() as conn:
                    ...
                    conn.commit()
                for ma_sv in sv_list:
                    append_security_audit("DELETE_ACCOUNT", ma_sv, "SUCCESS", "GUI Manager")
                self.after(0, lambda: (
                ...
                err_msg = str(e)
                for ma_sv in sv_list:
                    append_security_audit("DELETE_ACCOUNT", ma_sv, "FAILED", err_msg)
                self.after(0, lambda: messagebox.showerror("Lỗi", err_msg))
    ...
    def _open_barcode_folder(self):
        chosen_dir = filedialog.askdirectory(
            title="Chọn thư mục Barcode",
            initialdir=str(BARCODES_DIR),
        )
        if not chosen_dir:
            return

        folder_path = Path(chosen_dir)
        if not folder_path.exists():
            messagebox.showwarning("Lỗi", f"Không tìm thấy thư mục: {folder_path}")
            return
        ...
        try:
            if sys.platform.startswith("linux"):
                ...
                messagebox.showinfo("Thư mục Barcode", str(folder_path))
        except Exception as e:
