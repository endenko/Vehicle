...
class XeVaoPage(ctk.CTkFrame):
    ...
    def _execute_inbound_protocol(self, full_code: str):
        ...
        1. GUI quét mã → Gọi POST /register-student (httpx, non-blocking)
        2. ESP32 tự chụp ảnh → POST /process-plate?lane=IN
        3. GUI polling GET /lane-status?lane=IN để cập nhật kết quả
        """
            ...
            try:
                ...
                with httpx.Client(timeout=5.0) as client:
                    ...
                    if resp.status_code != 200:
                        ...
                        self.main_app.run_in_main_thread(
                            lambda m=msg: self._show_status_alert(f"⛔ Server: {m}", "red")
                        )
                        return
                    ...
                    try:
                        ...
                        if not ir_data.get("detected", False):
                            # Không có xe → cancel + show error
                            self.main_app.run_in_main_thread(
                                lambda: self._show_auto_close_popup("🚫 Không có xe tại cổng vào! IR Sensor không phát hiện xe.", "red", 3000)
                            )
                            return
                    except Exception:
                        # Nếu lỗi IR → vẫn tiếp tục (không block toàn bộ flow)
                        pass

                    # IR OK → Chờ ESP32 gửi ảnh
                    self.main_app.run_in_main_thread(
                        lambda: self.lbl_cam_status.configure(
                            text="✅ IR phát hiện xe. Chờ ESP32 chụp ảnh...",
                ...
                self.main_app.run_in_main_thread(
                    lambda err=e: self._show_status_alert(f"⛔ Lỗi kết nối server: {err}", "red")
                )
    ...
    def _poll_lane_status(self, lane: str, retries: int = 60):
        """Polling GET /lane-status?lane=X để cập nhật kết quả từ ESP32."""
        if retries <= 0:
