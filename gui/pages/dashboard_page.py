...
class DashboardPage(ctk.CTkFrame):
    ...
    def delete_selected_vehicle(self):
        ...
        if not selected:
            messagebox.showwarning("Xóa xe", "Vui lòng chọn 1 xe đang đỗ để xóa.")
            return

        values = self.tree.item(selected[0], "values")
        if not values:
            messagebox.showerror("Xóa xe", "Không đọc được dòng đã chọn.")
            return

        tx_id = values[0]
        plate = values[1] if len(values) > 1 else "-"
        if not messagebox.askyesno(
            "Xác nhận xóa",
            f"Bạn có chắc muốn xóa bản ghi xe đang đỗ ID={tx_id}, biển số {plate}?",
        ):
            return

        def _task():
            ok = delete_active_transaction(tx_id)

            def _done():
                if ok:
                    messagebox.showinfo("Xóa xe", "Đã xóa bản ghi thành công.")
                    self.refresh_data(self._current_keyword)
                else:
                    messagebox.showerror("Xóa xe", "Xóa thất bại. Bản ghi có thể đã được xử lý trước đó.")

            self.main_app.run_in_main_thread(_done)
