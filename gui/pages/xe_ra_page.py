...
class XeRaPage(ctk.CTkFrame):
                ...
                with httpx.Client(timeout=5.0) as client:
                    ...
                    try:
                        ...
                        if not ir_data.get("detected", False):
                            self.main_app.run_in_main_thread(
                                lambda: self._show_auto_close_popup("🚫 Không có xe tại cổng ra! IR Sensor không phát hiện xe.", "red", 3000)
                            )
                            self.main_app.run_in_main_thread(lambda: setattr(self, "_waiting", False))
                            return
                    except Exception:
                    ...
                    # IR OK → Chờ ESP32 gửi ảnh
                    self.main_app.run_in_main_thread(
                        lambda: self.lbl_cam_status.configure(
                            text="✅ IR phát hiện xe. Chờ ESP32 chụp ảnh cổng ra...",
                            text_color=AppStyle.STATUS_OK
                        )
                    )
    ...
    def _poll_lane_status_out(self, lane: str, student_info: dict, retries: int = 60):
        """Polling GET /lane-status?lane=OUT để cập nhật kết quả từ ESP32."""
        if retries <= 0:
            self.lbl_cam_status.configure(text="⏰ Timeout: ESP32 chưa gửi ảnh trong 60s", text_color=AppStyle.WARNING)
            self._waiting = False
            return
        ...
        def _check():
            ...
            try:
                with httpx.Client(timeout=3.0) as client:
                    resp = client.get(f"{API_ENDPOINT}/lane-status", params={"lane": lane})
                    return resp.json()
            except Exception:
    ...
    def _on_api_result_out(self, data: dict, student_info: dict):
        ...
        if code == 200:
            ...
            result = {
                ...
                "fee_info": {"total": fee, "base_fee": fee, "surcharge": 0,
                             "hours_parked": data.get("hours_parked", 0), "time_in_str": ""},
    ...
    def _fallback_local_outbound(self, student_info, active_tx, preview_plate):
        ...
        def _exit_hardware_and_vision():
            try:
                ...
                best_score = 0
                matched_plate = plate_captured or active_tx.get("BienSo") or active_tx.get("bien_so") or preview_plate

                if vehicles and plate_captured:
                    for vehicle in vehicles:
                        score = fuzz.ratio(plate_captured.upper(), vehicle["bien_so"].upper())
                        if score > best_score:
                            best_score = score
                            matched_plate = vehicle["bien_so"]
                elif matched_plate:
                    best_score = 100
    ...
    def _show_outbound_preview(self, student_info: dict, active_tx: dict | None, matched_plate: str):
        so_du = int(student_info.get("so_du", 0) or 0)
        time_in = self._format_time_value((active_tx or {}).get("ThoiGianVao") or (active_tx or {}).get("time_in"))
        ...
        self.res_surcharge.configure(text="-", text_color=AppStyle.TEXT_SECONDARY)
