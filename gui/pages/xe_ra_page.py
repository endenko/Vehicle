...
class XeRaPage(ctk.CTkFrame):
    ...
    def _on_barcode(self, code: str):
        ...
        if code:
            def _update():
                ...
                # Tự động xử lý xe ra khi quét được mã
                if not self._waiting:
                    self._auto_process_exit()
    ...
    # ── Auto process exit (Yêu cầu 5: tự động hóa) ───────────────────────────
    def _auto_process_exit(self):
        """Tự động xử lý xe ra khi quét được barcode."""
        ...
        # Dùng .after() thay vì time.sleep()
        self.after(2500, self._process_after_stable)

    def _process_after_stable(self):
        bc = self.ent_barcode.get().strip()
        plate = self.ent_plate.get().strip()

        # Hiển thị ảnh xe lúc VÀO để đối chiếu
        self._load_entry_image(plate)
    ...
    def _on_exit_result(self, result: dict, saved_path, plate):
        ...
        # Cảnh báo xe bị khóa (nhấp nháy đỏ)
        ...
        # Biển số không khớp → re-scan tự động sau 3s
        if result.get("rescan"):
            self._show_warn(f"⚠  BIỂN SỐ KHÔNG KHỚP\n{msg}\n\nYêu cầu đứng im, tự động quét lại sau 3 giây...",
        ...
        # Hiển thị ảnh ra
        if saved_path:
            self._display_exit_image(saved_path)
    ...
    # ── Re-scan (dùng .after, KHÔNG time.sleep) ──────────────────────────────
    def _schedule_rescan(self):
        """Delay 3 giây rồi tự động quét lại."""
        if self._rescan_job:
            self.after_cancel(self._rescan_job)
        self._rescan_job = self.after(3000, self._do_rescan)

    def _do_rescan(self):
        self._rescan_job = None
        self._hide_warn()
        self.lbl_cam_status.configure(
            text="🔄  Đang quét lại biển số...",
            text_color=AppStyle.STATUS_INFO)
        # Kích hoạt lại process
        self._auto_process_exit()
    ...
    def _do_blink(self):
        if self._blink_count >= 20:  # 10 giây (20 x 500ms)
            self._stop_blink_warning()
            return
        self._blink_state = not self._blink_state
        self._blink_count += 1
        if self._blink_state:
            self.warn_frame.configure(fg_color=AppStyle.DANGER)
        else:
            self.warn_frame.configure(fg_color="#7F1D1D")
    ...
    def _update_result_labels(self, result: dict, st: dict):
        ...
        if time_in_str and time_in_str != "-":
            if isinstance(time_in_str, str):
                try:
                    dt = datetime.fromisoformat(time_in_str)
                    time_in_str = dt.strftime("%H:%M  %d/%m/%Y")
                except ValueError:
                    pass
        self.res_time_in.configure(text=time_in_str)
        ...
        # Thời gian ra (Yêu cầu 6: hiển thị rõ ràng HH:MM DD/MM/YYYY)
