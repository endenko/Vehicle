            ...
            try:
                ...
                if current_name != self._last_file:
                    ...
                    if frame is not None:
                        content_sig = self._compute_content_signature(frame)
                        is_same_content = content_sig is not None and content_sig == self._last_content_signature
                        now = time.time()

                        # Không quét lại khi ảnh nội dung trùng nhau để tránh chớp/scan lặp.
                        if is_same_content:
                            time.sleep(self.poll_interval)
                            continue

                        # Chỉ cho phép quét ảnh mới sau mỗi cửa sổ cooldown.
                        if now - self._last_scan_ts < self.scan_cooldown:
                            time.sleep(self.poll_interval)
                            continue

                        self._last_content_signature = content_sig
