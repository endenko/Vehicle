...
class _BaseCameraHandler:
    def __init__(self, label: ctk.CTkLabel, on_detection: Optional[Callable[[Dict[str, object]], None]] = None):
        self.label = label
        self.running = False
    ...
    def _run_qr_refc_overlay(self, frame, emit_code: Optional[Callable[[str], None]]) -> None:
        if zbar_decode is None: return
        try: decoded = zbar_decode(frame)
        ...
        for item in decoded:
            ...
            dtype = "refc" if "REFC" in code.upper() else "qr"
            cv2.putText(frame, f"{dtype.upper()} 99%", (x, max(18, y - 6)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
