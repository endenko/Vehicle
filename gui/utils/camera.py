...
class _BaseCameraHandler:
    ...
    def _run_qr_refc_overlay(self, frame, emit_code: Optional[Callable[[str], None]]) -> None:
        ...
        # 4. Adaptive Threshold (Cực kỳ hiệu quả khi quét QR trên giấy lóa/bóng/không đều màu)
        adaptive = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 5)

        # 5. Khử nhiễu + Otsu (Tốt cho QR in trên giấy nháp/chất lượng in kém)
        blur = cv2.GaussianBlur(gray, (5, 5), 0)
        _, thresh_blur = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        
        # Danh sách các ảnh để thử quét (ưu tiên ảnh chất lượng cao nhất)
        candidates = [gray, adaptive, thresh_blur, enhanced, thresh, sharpened]
...
class IoTCameraHandler:
    def __init__(self, label: ctk.CTkLabel, capture_url: Optional[str] = None, on_detection=None, storage_dir: Optional[str] = None):
        ...
        if storage_dir:
            ...
            env_storage = os.getenv("ESP32_STORAGE_DIR", str(default_storage))
            self.storage_dir = Path(env_storage)
            
        self.poll_interval = float(os.getenv("ESP32_REFRESH_SECONDS", "1.0"))
        self.scan_cooldown = float(os.getenv("ESP32_SCAN_COOLDOWN_SECONDS", "3.0"))
        self._last_file: Optional[str] = None
        self._last_content_signature: Optional[int] = None
    ...
    def start(self):
        ...
        # Bỏ qua các ảnh cũ đã có sẵn trong thư mục lúc khởi động
        latest = self._get_latest_image_path()
        if latest:
            stat = latest.stat()
            self._last_file = f"{latest}:{stat.st_mtime_ns}:{stat.st_size}"
    ...
    def _compute_content_signature(self, frame: np.ndarray) -> Optional[int]:
        try:
    ...
    def _update(self):
        while self.base.running:
            try:
                ...
                stat = latest.stat()
                current_name = f"{latest}:{stat.st_mtime_ns}:{stat.st_size}"
