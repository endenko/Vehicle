...
def calc_fee(time_out: datetime = None) -> int:
    ...
    6:00 – 17:30  → 2,000đ
    17:30 – 23:00 → 3,000đ
    23:00 – 6:00  → từ chối (return -1) nếu TIME_LIMITED_23H_6H = 1
    """
    now = time_out or datetime.now()
    h = now.hour + now.minute / 60.0
    
    time_limited = os.getenv("TIME_LIMITED_23H_6H", "1")
    ...
    if time_limited == "0":
        return 3000  # Chế độ test: vẫn cho phép hoạt động ngoài giờ
...
def lookup_by_barcode(barcode: str) -> dict:
    ...
    # Chuẩn hóa mã REFC-* về MSSV để tra cứu thống nhất cho cả cổng vào/ra.
    refc_match = None
    try:
        import re
        refc_match = re.search(r"REFC\s*[-_: ]?\s*([A-Z0-9-]{3,})", barcode.upper())
    except Exception:
        refc_match = None
    if refc_match:
        normalized_id = refc_match.group(1).strip().upper().replace("-", "")
        if normalized_id:
            barcode = normalized_id
        
    qr_data = None
    # Xử lý nếu là mã QR (chứa JSON payload)
    if "MSSV" in barcode:
        # Chuẩn hóa smart quotes do máy quét vạch / Unikey gây ra
        clean_barcode = barcode.replace("“", '"').replace("”", '"').replace("‘", "'").replace("’", "'")
        try:
            import json
            qr_data = json.loads(clean_barcode)
            raw_mssv = qr_data.get("MSSV", barcode)
            barcode = str(raw_mssv).strip() if raw_mssv is not None else barcode
        except Exception:
            try:
                import ast
                qr_data = ast.literal_eval(clean_barcode)
    ...
    # Đảm bảo barcode luôn là chuỗi để tránh crash DB
    barcode = str(barcode).strip()
...
def is_vehicle_in_lot(ma_rfid: str = None, bien_so: str = None) -> Optional[dict]:
    ...
    if ma_rfid:
        ...
        active = db_manager.execute_query(query, (ma_rfid,))
        if active: return active[0]
        #active = db_manager.get_active_entry_by_rfid(ma_rfid)
        #if active: return active
...
def process_entry(barcode: str, bien_so: str, anh_path: str = "") -> dict:
    ...
    if is_vehicle_in_lot(ma_rfid=ma_rfid):
        return {"ok": False, "message": "Xe này đã có lượt vào đang đỗ.", "student": student_info}
    ...
    return {"ok": False, "message": "Lỗi ghi log vào DB.", "student": student_info, "image_path": anh_path}
...
def process_exit(
    ...
    # 3. Lấy đúng lượt vào đang đỗ theo RFID rồi fuzzy match biển số lúc vào
    ...
    entry_plate = (active_tx.get("BienSo") or active_tx.get("bien_so") or "").strip()
    entry_time = active_tx.get("ThoiGianVao") or active_tx.get("thoi_gian_vao")

    plate_match_score = 0
    matched_plate = entry_plate or (bien_so_quet or "").strip()

    if not force and not bien_so_quet:
        return {
            "ok": False,
            "message": "⚠ Không đọc được biển số lúc ra. Vui lòng chụp lại ảnh.",
            "student": student_info,
            "plate_score": 0,
            "rescan": True,
        }

    if matched_plate and bien_so_quet and _HAS_FUZZ:
        best, score = fuzz_process.extractOne(
            bien_so_quet.upper(), [matched_plate.upper()], scorer=fuzz.partial_ratio
        )
        plate_match_score = score or 0
        if (not force) and score < FUZZY_THRESHOLD:
            return {
                "ok": False,
                "message": (f"⚠ Biển số không khớp! Quét: {bien_so_quet} | "
                            f"Biển số lúc vào: {matched_plate} ({score}%)"),
                "student": student_info,
                "plate_score": score,
                "rescan": True,
            }
...
def delete_active_transaction(transaction_id: int) -> bool:
    """Xóa một dòng đang đỗ theo ID (dùng cho xử lý trùng/lỗi dữ liệu)."""
    try:
    ...
    db_manager.connect()
    if db_manager.engine == "sqlserver":
        query = "DELETE FROM LichSuVaoRa WHERE ID = ? AND TrangThai = N'Đang đỗ'"
    else:
        query = "DELETE FROM LichSuVaoRa WHERE ID = ? AND TrangThai = 'Đang đỗ'"
    db_manager.execute_query(query, (tx_id,))

    verify = db_manager.execute_query("SELECT ID FROM LichSuVaoRa WHERE ID = ?", (tx_id,))
    return len(verify) == 0
