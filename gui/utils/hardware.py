...
async def check_ir_sensor_async(direction: str = "in", timeout: float = 3.0) -> bool:
    ...
    Returns:
        True nếu IR phát hiện có xe (bị chắn) HOẶC nếu không kết nối được ESP32
             (fail-open: lỗi kết nối → giả định có xe → cho flow tiếp tục)
        False CHỈ khi ESP32 phản hồi rõ ràng {"detected": false}
    """
    url = URL_IR_SENSOR_IN if direction == "in" else URL_IR_SENSOR_OUT
    if MOCK_HARDWARE:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            print(f"[!] IR Sensor ({direction}): HTTP {resp.status_code} — giả định có xe")
            return True  # Fail-open: HTTP error → giả định có xe
    except httpx.ConnectError as e:
        print(f"[!] IR Sensor ({direction}): Không kết nối được ESP32 ({url}) — giả định có xe")
        return True  # Fail-open: ESP32 offline → giả định có xe, tiếp tục flow
    except httpx.TimeoutException:
        print(f"[!] IR Sensor ({direction}): Timeout kết nối ESP32 — giả định có xe")
        return True  # Fail-open
    except Exception as e:
        print(f"[!] IR Sensor ({direction}): Lỗi [{type(e).__name__}] {e} — giả định có xe")
        return True  # Fail-open: mọi lỗi khác → tiếp tục
