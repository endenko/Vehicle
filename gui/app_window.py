...
if (__package__ or "").startswith("gui"):
    ...
    from .utils.db_local import backup_primary_database
else:
    from pages.dashboard_page import DashboardPage
    from pages.info_page import InfoPage
    from pages.log_history_page import LogHistoryPage
    ...
    from pages.xe_vao_page import XeVaoPage
    from styles import AppStyle
    from gui.utils.db_local import backup_primary_database
...
class MainApp(ctk.CTk):
    def __init__(self):
        ...
        # ── Pages ─────────────────────────────────────────────────────────────
        self.dashboard_page = DashboardPage(self.tabview.tab("Dashboard"), self)
        ...
        self.info_page = InfoPage(self.tabview.tab("Thong Tin"), self)
        ...
        threading.Thread(target=backup_primary_database, daemon=True).start()

        # KHỞI ĐỘNG CAMERA SAU KHI GUI ĐÃ HIỆN LÊN
        print("[GUI] Đang đợi giao diện hiển thị để khởi động Camera...")
        self.after(1200, self._auto_start_cameras)
    ...
    def _auto_start_cameras(self):
        if self.winfo_exists() and self.tabview.get() == "Xe Vao":
