...
## Cấu hình Thiết bị ngoại vi (Hardware Integration)
Hệ thống hỗ trợ kết nối với Cảm biến hồng ngoại (IR), ESP32-CAM và Barie tự động. Hiện tại hệ thống đang ở chế độ **Giả lập (Mock)** để bạn có thể chạy thử GUI mà không cần phần cứng.

### 1. Cách chuyển sang Hardware thật:
Để kết nối thiết bị thật, bạn hãy thay đổi cấu hình trong file `.env` (hoặc trực tiếp trong `smart_parking/config.py`):

```env
# Tắt chế độ giả lập (yes -> no)
MOCK_HARDWARE=no

# Cấu hình URL cho các thiết bị
URL_IR_SENSOR_IN=http://192.168.1.100/ir_in
URL_IR_SENSOR_OUT=http://192.168.1.100/ir_out
URL_ESP32_CAPTURE_IN=http://192.168.1.101/capture
URL_ESP32_CAPTURE_OUT=http://192.168.1.102/capture
URL_BARRIER_OPEN=http://192.168.1.100/open
```

### 2. File quản lý Logic:
- Toàn bộ logic giao tiếp phần cứng được tập trung tại file: [gui/utils/hardware.py](file:///home/minhviet/Documents/TestThuGui/gui/utils/hardware.py).
- Nếu bạn có giao thức kết nối khác (ví dụ qua Serial/MQTT), hãy chỉnh sửa các hàm `trigger_ir_sensor`, `request_esp32_capture`, và `open_barrier` trong file này.
